...
def get_longcat_client():
    """Initialize OpenAI-compatible client for LongCat with proper error handling."""
    try:
        from openai import OpenAI
    except ImportError:
        raise ImportError("OpenAI package not installed. Run: pip install openai>=1.30.0")
    
    api_key = current_app.config.get('LONGCAT_API_KEY')
    base_url = current_app.config.get('LONGCAT_BASE_URL', 'https://api.longcat.chat/openai/')
    ...
    if not api_key or api_key == 'your_longcat_api_key_here' or len(api_key) < 20:
        if flask_env == 'development':
            print("[ERROR AI_SERVICE] Invalid or missing API key!")
        raise ValueError("LONGCAT_API_KEY not configured. Please set it in your environment variables with a valid API key.")
    
    try:
        # LongCat uses the OpenAI SDK with a custom base URL.
        if flask_env == 'development':
            print("[DEBUG AI_SERVICE] Creating LongCat client...")
        client = OpenAI(api_key=api_key, base_url=base_url)
        if flask_env == 'development':
            print("[DEBUG AI_SERVICE] LongCat client created successfully!")
        return client
...
def analyze_resume_text(raw_text, parsed_data):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model=current_app.config.get('LONGCAT_MODEL', 'longcat-flash-chat'),
